...
SELECT
    ...
    , CASE
    ps.hospital
	, WHEN 'Kings College' THEN 'Urban' ELSE 'Rural'
...
SELECT
	...
	, ps.Hospital
	, CASE
     WHEN ps.Ward LIKE '%Surgery' THEN 'Surgical' AS WardType
FROM
	dbo.PatientStay ps
ORDER BY
	WardType;
...
SELECT
	...
	, ps.Tariff
	, CASE
      WHEN ps.Tariff >=7 'High Tarrif' THEN
       WHEN <=4 'Low Tarrif' END  AS PatientTariffGroup
FROM
	dbo.PatientStay ps
ORDER BY
	PatientTariffGroup
